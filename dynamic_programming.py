...
def find_LPS_length(st):
    n = len(st)
    dp = [[-1 for _ in range(n)] for _ in range(n)]
    return find_LPS_length_dp(dp, st, 0, n - 1)


def find_LPS_length_dp(dp, st, startIndex, endIndex):
    if startIndex > endIndex:
        return 0
    ...
    if dp[startIndex][endIndex] == -1:
        if st[startIndex] == st[endIndex]:
            remainingLength = endIndex - startIndex - 1
            if remainingLength == find_LPS_length_dp(
                dp, st, startIndex + 1, endIndex - 1
            ):
                dp[startIndex][endIndex] = remainingLength + 2
                return dp[startIndex][endIndex]

        c1 = find_LPS_length_dp(dp, st, startIndex + 1, endIndex)
        c2 = find_LPS_length_dp(dp, st, startIndex, endIndex - 1)
